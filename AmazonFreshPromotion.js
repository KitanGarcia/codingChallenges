...
/*
function promotion(codeList, cart) {
  //loop through cart and through codelist
  //once codelist[i] is satisfied, move on to the next one
  //if not satisfied, return 0
  let p1 = 0;//track in cart 
  let p2 = 0;
  for (let i = p1; i < cart.length; i++) {
    }
  }
  return 1;
/*
 ...
 * \W: non-alphanumeric characters (ie. punctuation, other)
 * \s: whitespace
 * \S: non-whitespace
 * |: or
 */

//assuming all fruits are 1 word long
/*
import re
def promotionPy(codeList, cart) {
  pattern = ""
  pattern += "\S*"
  ...
  printf(f"pattern = {pattern}")

  string = " ".join(cart)

  printf(f"string = {string}")
...
console.log(promotion(codeList, shoppingCart), 1);
